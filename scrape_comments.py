...
async def fetch_public_comment(session: aiohttp.ClientSession, comment_stub: PublicCommentListItem) -> PublicComment | None:
    headers = {
        "User-Agent": "Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:109.0) Gecko/20100101 Firefox/110.0",
    }
        ...
        async for page in public_comments_iterable:
            ...
            for chunk in page_chunks:
                result = await asyncio.gather(
                    *(fetch_public_comment(session, comment) for comment in chunk)
                )
